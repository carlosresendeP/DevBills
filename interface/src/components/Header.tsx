...
export const Header = () => {
    ...
    const renderAvatar = () => {
        ...
        if(authState.user.photoURL) {
            return <img src={authState.user.photoURL} alt={authState.user.displayName || 'User Avatar'} 
            className="w-8 h-8 rounded-full object-cover"/>
        }

        return (
        <div className="w-8 h-8 rounded-full bg-primary-500 flex items-center justify-center text-white font-medium">{authState.user.displayName?.charAt(0).toUpperCase()}</div>)
    }
                        ...
                        {isAuthenticated ?(
                            ...
                            <nav className="space-y-2">
                                {navLinks.map((link) => (
                                    ...
                                    className={`block px-3 p-2 rounded-lg ${pathname === link.path 
                                        ? 'text-primary-500 bg-primary-500/10 rounded-md h-10 px-3 py-2' 
                                        : 'text-gray-300 h-10 px-3 py-2 hover:text-primary-500 hover:bg-primary-500/10 rounded-md'}`}>
                                        {link.name}
                                    </Link>
